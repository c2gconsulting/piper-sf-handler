...
//variables
var hostname = "https://salesdemo4.successfactors.com";
...
var connect = function( clientID , userP , passW , callback){
	  var options = {
      hostname: hostname,
      path: encodeURI ( "/odata/v2/User?$format=json&$filter=userId in 'ctse1','mhuang1', 'flynch1' , 'cgrant1' & $select=username,userId" ),
      method: 'GET',
...
var getLeaveQuota = function (clientID, userP, passW , callback ) {
    ...
    var options = {
        hostname: "salesdemo4.successfactors.com",
...
getLeaveQuota( "acetxe" , "cgrant" , "demo101" , function ( res ) {
   ...
   if ( res.d.results.length > 0 ){
       for ( a = 0 ; a<res.d.results.length;a++){
            ...
            console.log ( "Balance for "+leaveQuotaBalance.userId+" is "+ leaveQuotaBalance.balance 
        +" for leave type "+leaveQuotaBalance.timeAccountType);
...
getLeaveQuota( "acetxe" , "admin" , "demo101" , function ( res ) {
   ...
   if ( res.d.results.length > 0 ){
       for ( a = 0 ; a<res.d.results.length;a++){
            leaveQuotaBalance = res.d.results[a];
            console.log ( "Balance for "+leaveQuotaBalance.userId+" is "+ leaveQuotaBalance.balance 
        +" forleave type "+leaveQuotaBalance.timeAccountType);
